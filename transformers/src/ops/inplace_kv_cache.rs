...
/// NNEF (de)serialization for the fused `InPlaceKvSdpa` op.
pub fn register(registry: &mut Registry) {
    registry.register_dumper(ser_inplace_kv_sdpa);
    registry.register_primitive(
        "tract_transformers_inplace_kv_sdpa",
        &[
            TypeName::Scalar.tensor().named("q"),
            TypeName::Scalar.tensor().named("k"),
            TypeName::Scalar.tensor().named("v"),
            TypeName::Integer.named("axis"),
            TypeName::Logical.named("causal"),
...
fn ser_inplace_kv_sdpa(
    ...
    let mut attrs = vec![("axis", numeric(op.axis)), ("causal", logical(op.causal))];
    if let Some(scale) = op.scale {
        attrs.push(("scale", numeric(scale)));
    }
    Ok(Some(invocation("tract_transformers_inplace_kv_sdpa", &[q, k, v], &attrs)))
}

fn de_inplace_kv_sdpa(
    ...
    let causal: bool = invocation.named_arg_as(builder, "causal")?;
    let scale: Option<f32> = invocation.get_named_arg_as(builder, "scale")?;
    builder.wire(InPlaceKvSdpa { axis, causal, scale }, &[q, k, v])
}
...
mod tests {
    ...
    // NNEF ser/de round-trip: the fused op survives write_to_tar -> model_for_read
    // with its attributes intact.
    #[test]
    fn inplace_kv_sdpa_nnef_round_trip() -> TractResult<()> {
        ...
        let newf: TVec<TDim> = tvec![dim(b), dim(hkv), s.into(), dim(d)];
        let q = model.add_source("q", f32::fact(&qf))?;
        let k = model.add_source("k", f32::fact(&newf))?;
        let v = model.add_source("v", f32::fact(&newf))?;
        let o = model.wire_node(
            "attn",
            InPlaceKvSdpa { axis: 2, causal: true, scale: Some(0.125) },
            &[q, k, v],
        )?;
        model.select_output_outlets(&o)?;
        ...
        let n = reloaded
            .nodes()
            .iter()
            .find(|n| n.op_is::<InPlaceKvSdpa>())
            .context("InPlaceKvSdpa survived the round-trip")?;
        let op = n.op_as::<InPlaceKvSdpa>().unwrap();
        assert_eq!(op.axis, 2);
        assert!(op.causal);
