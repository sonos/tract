...
impl EvalOp for Sdpa {
    ...
    fn eval(&self, mut inputs: TVec<TValue>) -> TractResult<TVec<TValue>> {
        ...
        if q.rank() == 4 {
            ...
            if q_heads > k_heads {
                ...
                mask = mask
                    ...
                    .transpose()?;
                final_reshape = Some((vec![*b, *k_heads, g, *seq_len, *k_dims], vec![*b, *q_heads, *seq_len, *k_dims]));
            }
        }
        ...
        if let Some((_, to)) = final_reshape {
            return Ok(tvec!(output.remove(0).into_tensor().into_shape(&to)?.into()));
        }
        Ok(output)
    }
}
...
impl TypedOp for Sdpa {
    ...
    fn declutter(
        ...
        let new_model = new_model.into_decluttered()?;
        
        let new_op = Sdpa { subgraph: Some(new_model), ..self.clone() };
