...
/// Streaming on the FFT axis itself must be rejected with a clear
/// error: per-pulse FFT on the FFT axis is meaningless (FFT needs
/// every sample on that axis at once). The pulsifier bails fast.
#[test]
fn fft_pulse_on_fft_axis_errors() {
    ...
    let msg = format!("{err:#}");
    assert!(
        msg.contains("FFT axis") || msg.contains("FFT needs"),
        "expected a clear FFT-axis-streaming error, got: {msg}"
    );
}
