#!/usr/bin/env python3
"""Generate io.npz and r_full.dat for ex13-rel-pos-skew-window.
...
With left_chunks=1 each query can attend to its own chunk AND the preceding
chunk, so the key window size W = (left_chunks+1)*P = 4 per pulse.

The position-score path (same as ex12 at batch time):
...
At pulse time the PROBLEM is:
    content_scores  [1, P=2, W=4]   (K includes left_chunks*P buffered keys)
    pos_scores      [1, P=2, P=2]   (R sliced using T=P from input shape)
    Add -> Broadcast(W=4, P=2) -- pulsification fails
