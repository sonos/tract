...
S_audio, Dh, P_token, left_chunks, B = 64, 4, 2, 3, 1
# Two stride-2 subsamples: S → ceil(S/2) → ceil(ceil(S/2)/2)
T_mid = (S_audio + 1) // 2   # 32
T = (T_mid + 1) // 2          # 16
W = (left_chunks + 1) * P_token  # 8
R = 2 * T - 1                 # 31
center = T                     # 16 — but we use 32 in graph for larger table
...
# r_full is [63, 4] (center=32, so table covers distances -(T-1) to +(T-1) with room)
R_full = 63
r_full = rng.standard_normal((R_full, Dh)).astype(np.float32)
...
print(f"io.npz  q_audio={q_audio.shape} T={T} output={output.shape}")

data = r_full.astype("<f4").tobytes()
dims = struct.pack("<8I", R_full, Dh, 0, 0, 0, 0, 0, 0)
