#!/usr/bin/env python3
"""Generate io.npz for the banded-causal synthetic.

Causal banded mask: row at chunk c attends to chunks {c, c-1, ..., c-L}
(past-only).  For L = 1 and P = 2 this is a P-block upper bidiagonal.
...
mask      = ((diff >= -L) & (diff <= 0)).astype(np.float32)
