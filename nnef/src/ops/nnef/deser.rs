...
pub fn variable(builder: &mut ModelBuilder, invocation: &ResolvedInvocation) -> TractResult<Value> {
    ...
    if let Some(bwf) =
        tensor_dense.to_scalar::<Opaque>().ok().and_then(|o| o.downcast_ref::<BlobWithFact>())
    {
...
pub fn reshape(builder: &mut ModelBuilder, invocation: &ResolvedInvocation) -> TractResult<Value> {
    ...
    let replacement_dense = replacement.try_as_dense()?;
    let mut replacement: TVec<TDim> = replacement_dense.as_slice::<TDim>()?.into();
