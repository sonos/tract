...
mod test {
    ...
    fn test_eval_nhwc_1() -> TractResult<()> {
        ...
        let res = op.eval(tvec!(
            Tensor::zero::<f32>(&[1, 2, 2, 2]).unwrap().into(),
            Tensor::zero::<f32>(&[2, 2, 2, 1]).unwrap().into(),
        ))?;
    ...
    fn test_eval_nhwc_batch() {
        ...
        let result = op
            ...
            .unwrap();
        assert_eq!(result, tvec!(tensor4(&[[[[2.0f32]]], [[[0.0f32]]]])));
    }
    ...
    fn test_eval_ntc_batch() {
        ...
        let result = op
            .eval(tvec!(tensor3(&[[[2.0f32]], [[0.0f32]]]).into(), tensor3(&[[[1.0f32]]]).into()))
            .unwrap();
        assert_eq!(result, tvec!(tensor3(&[[[2.0f32]], [[0.0f32]]])));
    }
    ...
    fn test_eval_ntc_channel() {
        ...
        let result = op
            ...
            .unwrap();
        assert_eq!(result, tvec!(tensor3(&[[[2.0f32]]])));
    }
}
