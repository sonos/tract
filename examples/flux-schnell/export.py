...
def main():
    ...
    # --- Transformer (Flux) ---
    # Wrap with latent packing, img_ids/txt_ids generation so the ONNX model
    # has a clean interface: (latent, t5_embeds, clip_pooled, timestep)
    ...
    class FluxTransformerWrapper(torch.nn.Module):
        ...
        def forward(self, latent, encoder_hidden_states, pooled_projections, timestep):
            ...
            # Pack latents into patch sequence
    ...
    sample_latent = torch.randn(1, 16, 128, 128)
    sample_t5 = torch.randn(1, 256, 4096)
    sample_pooled = torch.randn(1, 768)
    sample_t = torch.tensor([500.0])
