...
impl RawSimplePlan {
    pub fn new(model:&Model, inputs: &[impl AsRef<str>], outputs: &[impl AsRef<str>]) -> Result<RawSimplePlan> {
        let input_ids:Vec<usize> = inputs.iter().map(|n| Ok(model.node_by_name(n.as_ref())?.id)).collect::<Result<_>>()?;
        let output_ids:Vec<usize> = outputs.iter().map(|n| Ok(model.node_by_name(n.as_ref())?.id)).collect::<Result<_>>()?;
...
impl SimpleState {
    ...
    pub fn set_inputs(&mut self, inputs: Vec<Tensor>) -> Result<()> {
        ...
        inputs.into_iter().zip(plan.input_ids.iter()).for_each(|(t, &id)| values[id] = Some(vec!(t.into())));
        Ok(())
    }
