...
pub fn handle(params: Parameters, assert_outputs:Option<Vec<TensorFact>>, options: DisplayOptions) -> CliResult<()> {
    let tfd = params.tfd_model;

    let display_graph = DisplayGraph::from_model_and_options(&tfd, options)?.with_graph_def(&params.graph)?;
