    ...
    #[cfg(feature = "bench-suite")]
    {
        app = app.subcommand(
            clap::Command::new("bench-suite")
                ...
                .arg(arg!(--"skip-cpu" "Skip CPU runs: drop the runtime-less (net) benches and the cpu runtime of the rest. For GPU-only devices whose CPU is redundant with another arm64/x86 box."))
                .arg(arg!(--"skip-runtimes" "Run only the plain-CPU (runtime-less) net benches, dropping every bench that declares a runtime sweep (the LLM + accelerator/large-model suite). For small embedded CPU targets."))
