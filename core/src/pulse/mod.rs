...
mod tests {
    ...
    fn proptest_regular_against_pulse(model: Model, pulse:usize, input_array: ArrayD<f32>, axis: usize) -> TestCaseResult {
    ...
    proptest! {
        ...
        fn proptest_crop(pulse in 1i32..3, input_len in 0i32..10, begin in 0i32..3, end in 0i32..3) {
            ...
            let input = arr1(&*(0..input_len).map(|x| x as f32).collect::<Vec<_>>());
            proptest_regular_against_pulse(model.clone(), pulse as _, input.into_dyn(), 0)?;
        }
        ...
        fn proptest_pad(pulse in 1i32..3, input_len in 0i32..10, begin in 0i32..3, end in 0i32..3) {
            ...
            let _ = model
                .add_source_fact("a", TensorFact::dt_shape(f32::datum_type(), shapefact!(S)))
                .unwrap();
            model.chain("pad", Box::new(Pad::new(vec![(begin as _, end as _)], PadMode::Constant(12.0)))).unwrap();
