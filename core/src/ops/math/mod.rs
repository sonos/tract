...
element_wise!(tanh, Tanh,
 [f16] => |_, xs| { tract_linalg::routines::kernel_f16(tract_linalg::routines::Routine::Tanh).run(xs) },
 [f32] => |_, xs| { tract_linalg::routines::kernel_f32(tract_linalg::routines::Routine::Tanh).run(xs) },
...
element_wise!(erf, Erf,
 [f32] => |_, xs| { tract_linalg::routines::kernel_f32(tract_linalg::routines::Routine::Erf).run(xs) },
 [f16] => |_, xs| {
     let mut f32s = xs.iter().map(|x| x.to_f32()).collect_vec();
     tract_linalg::routines::kernel_f32(tract_linalg::routines::Routine::Erf).run(&mut f32s)?;
