...
impl TypedOp for Gather {
    ...
    fn axes_mapping(
        ...
        if !inputs[0].is_plain() {
            return AxesMapping::disconnected(inputs, &[&inputs[0].datum_type.fact(&[0i64.to_dim()])]);
        }
        ...
        for k in 0..self.axis {
            axes.push(crate::axes::Axis::new(alphabet.next().unwrap(), 2, 1).input(0, k).output(0, k));
        }
        ...
        for k in self.axis + 1..data_rank {
            ...
            axes.push(crate::axes::Axis::new(alphabet.next().unwrap(), 2, 1).input(0, k).output(0, out_pos));
        }
        for k in 0..indices_rank {
            let out_pos = self.axis + k;
            axes.push(crate::axes::Axis::new(alphabet.next().unwrap(), 2, 1).input(1, k).output(0, out_pos));
        }
