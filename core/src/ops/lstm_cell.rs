...
impl EvalOp for LstmEpilogue {
    ...
    fn eval(&self, inputs: TVec<TValue>) -> TractResult<TVec<TValue>> {
        ...
        use tract_linalg::routines::{Routine, kernel_f16, kernel_f32};
        match inputs[0].datum_type().unquantized() {
            DatumType::F32 => {
                self.eval_t::<f32>(inputs, kernel_f32(Routine::Sigmoid), kernel_f32(Routine::Tanh))
            }
            DatumType::F16 => {
                self.eval_t::<f16>(inputs, kernel_f16(Routine::Sigmoid), kernel_f16(Routine::Tanh))
            }
