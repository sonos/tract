...
impl Inference {
    fn unify_scanning_tensor_fact(
        ...
        if let Some(rank) = rank {
            outer
                .shape
                .unify_with(&ShapeFact::closed(tvec!(GenericFact::Any; rank as usize)))?;
            inner
                .shape
                .unify_with(&ShapeFact::closed(tvec!(GenericFact::Any; rank as usize)))?;
            for axis in 0..rank {
    ...
    fn unify_facts(
        ...
        let hidden_state_len = body_inputs - self.scan_input_axes.len() - self.closure_inputs;
        ...
        for i in 0..self.scan_input_axes.len() {
            ...
            let axis = self.scan_input_axes[i];
            Self::unify_scanning_tensor_fact(incoming, inner, axis)?;
        }
        for i in 0..self.closure_inputs {
            let id = hidden_state_len + self.scan_input_axes.len() + i;
