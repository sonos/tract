    ...
    fn eval(&self, mut inputs: TVec<Arc<Tensor>>) -> TractResult<TVec<Arc<Tensor>>> {
        ...
        let iters = if self.prune_scanning_dim {
            ...
            inputs[0].shape()[first_input_axis].div_ceil(
                self.plan.model().input_fact(hidden_state_len)?.shape.dim(first_input_axis).to_integer()?
                    as usize,
            )
        };
        ...
        for i in 0..iters {
            ...
            let mut iter_outputs = self.plan.run(iter_inputs).chain_err(|| "Evaluating inner body")?;
