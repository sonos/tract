...
impl TypedOp for Const {
    ...
    //                 return Ok(None);
    //             }
    //         } else if let Some(einsum) = snode.op_as::<EinSumMatMul>() {
    //             if !einsum.ready_for_mm_determination {
    //                 return Ok(None);
    //             }
    //             matmuls.push((einsum, succ.slot));
    //         } else {
    //             return Ok(None);
    //         }
    //     }
    ...
    //     ensure!(matmuls.iter().map(|linear| linear.0.m_axis().inputs[0][0]).all_equal());
    //     ensure!(matmuls.iter().map(|linear| linear.0.k_axis().inputs[0][0]).all_equal());
