    ...
    fn codegen(
        ...
        if let Some(linalg_bin_op) = self.0.as_linalg_binop() {
            ...
            if by_scalar_should_be_efficient & can_eval_in_a & !op_is_quant {
                rule_if_some!(
                    func = tract_linalg::routines::bin(
                        tract_linalg::routines::Routine::BinByScalar(actual_linalg_op),
                        dt
                    )
                );
            ...
            if unicast_should_be_efficient & can_eval_in_a & !op_is_quant {
                rule_if_some!(
                    func = tract_linalg::routines::bin(
                        tract_linalg::routines::Routine::BinUnicast(actual_linalg_op),
                        dt
                    )
                );
                let eval_fn = Arc::from(func);
...
mod tests {
    ...
    fn opt_bin_unicast_falls_back_on_non_natural_strides() {
        ...
        let linalg_fn = tract_linalg::routines::bin(
            tract_linalg::routines::Routine::BinUnicast(BinOp::Add),
            f32::datum_type(),
        )
    ...
    fn zero_sized_outer_dim_is_a_noop() {
        ...
        let linalg_fn = tract_linalg::routines::bin(
            tract_linalg::routines::Routine::BinByScalar(BinOp::Add),
            f32::datum_type(),
        )
