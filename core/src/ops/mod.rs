...
pub trait OpHash {
    fn dhash(&self, state: &mut dyn std::hash::Hasher);
}
...
/// A base operation
pub trait Op:
    fmt::Debug + dyn_clone::DynClone + Send + Sync + 'static + Downcast + StatefullOp + OpHash
{
...
pub trait PulsedOp:
    Op + fmt::Debug + dyn_clone::DynClone + Send + Sync + 'static + Downcast + StatefullOp + OpHash
{
...
impl<O: std::hash::Hash> OpHash for O {
    fn dhash(&self, state: &mut dyn std::hash::Hasher) {
