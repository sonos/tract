...
impl<F, O, C> Deref for ModelPatch<F, O, C>
where
    F: Fact + Clone + 'static,
    O: Display + Debug + AsRef<dyn Op> + AsMut<dyn Op> + Clone + 'static,
    C: ModelChecker<F, O>,
    ModelImpl<F, O, C>: ModelSpecialOps<F, O>,
{
    type Target = ModelImpl<F, O, C>;
    fn deref(&self) -> &ModelImpl<F, O, C> {
        &self.model
    }
}

impl<F, O, C> DerefMut for ModelPatch<F, O, C>
where
    F: Fact + Clone + 'static,
    O: Display + Debug + AsRef<dyn Op> + AsMut<dyn Op> + Clone + 'static,
    C: ModelChecker<F, O>,
    ModelImpl<F, O, C>: ModelSpecialOps<F, O>,
{
    fn deref_mut(&mut self) -> &mut ModelImpl<F, O, C> {
        &mut self.model
    }
}

impl<F, O, C> ModelPatch<F, O, C>
where
    F: Fact + Clone + 'static,
    O: Display + Debug + AsRef<dyn Op> + AsMut<dyn Op> + Clone + 'static,
    C: ModelChecker<F, O>,
    ModelImpl<F, O, C>: ModelSpecialOps<F, O>,
{
    ...
    pub fn replace_single_op<IO: Into<O>>(
        patched_model: &ModelImpl<F, O, C>,
        node: &BaseNode<F, O>,
        inputs: &[OutletId],
        new_op: IO,
    ) -> TractResult<ModelPatch<F, O, C>> {
    ...
    pub fn apply(self, target: &mut ModelImpl<F, O, C>) -> TractResult<()> {
        ...
        for node in patch.nodes {
            if <ModelImpl<F, O, C>>::is_source(node.op()) {
                continue;
            }
