...
pub fn fuse_move_axis(
    ...
    let simpl_op = GpuAxisOp::simplify_axis_op(axis_op.inner.clone(), in_shape.dims(), axis_op.backend_name, axis_op.dispatch);
    if simpl_op != *axis_op {
    ...
    if let (AxisOp::Move(from_1, to_1), AxisOp::Add(ax)) = (
        ...
        if ax == from_1 {
            ...
            let out =
                patch.wire_node(cursor.name.clone(), GpuAxisOp::new(AxisOp::Add(to_1), axis_op.backend_name, axis_op.dispatch), &inputs)?;
