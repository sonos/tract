        ...
        if translatable {
            ...
            let outlet_ids: TVec<OutletId> = if let Some(op) = node.op_as::<PrefixMatMul>() {
                ...
                let op: Box<dyn TypedOp> = if let Some(op) = node.op_as::<ElementWiseOp>() {
                    ...
                    Box::new(tract_gpu::ops::change_axes::GpuAxisOp::from_tract_core_with_fact(op.clone(), in_fact, "Metal", crate::kernels::array::metal_copy_nd_dispatch))
        ...
        MetalGemmImplKind::Ggml => {
            ...
            if op.transpose_a {
                ...
                let perm_a_op = tract_gpu::ops::change_axes::GpuAxisOp::new(AxisOp::Move(
                    rank - 2,
                    rank - 1,
                ), "Metal", crate::kernels::array::metal_copy_nd_dispatch);
