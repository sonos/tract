...
// Residual-add fused variants: the input is the elementwise sum of two
// same-shape tensors, computed in the INPUT precision (identical to the
// standalone Add dispatch, so the fusion is bit-exact) and also written out
// (the residual stream needs the sum too). Everything else matches the
// non-add kernels above.
template <typename FI, typename FO>
[[kernel]] void rms_norm_add_nd3(device const void *input_b,
    ...
    device const FI *input = (device const FI *)input_b;
    device const FI *res = (device const FI *)res_b;
    float eps = ((constant float *)eps_b)[0];
    device FO *output = (device FO *)output_b;
    device FI *sum_out = (device FI *)sum_b;
    ...
    for (size_t i = tpitg; i < dim; i += ntg) {
        auto idx = base_idx + i * strides[1];
        const FI v = input[idx] + res[idx];
        output[idx] = static_cast<FO>(static_cast<float>(v) * norm);
    }
}

template <typename I4, typename O4>
[[kernel]] void rms_norm_add_nd2_l4(device const char *input_b,
    ...
    device const I4 *x = (device const I4 *)(input_b + tgpig * outer_stride);
    device const I4 *r = (device const I4 *)(res_b + tgpig * outer_stride);
    device I4 *s = (device I4 *)(sum_b + tgpig * outer_stride);
    float eps = ((constant float *)eps_b)[0];
    float sumf = 0.0f;
    ...
    for (size_t i = tpitg; i < n_div_4; i += ntg) {
        const I4 v = x[i] + r[i];
        s[i] = v;
        float4 el = static_cast<float4>(v);
        sumf += dot(el, el);
    }
    sumf = simd_sum(sumf);
    ...
    device O4 *y = (device O4 *)output_b + tgpig * n_div_4;
    for (size_t i = tpitg; i < n_div_4; i += ntg) {
        y[i] = static_cast<O4>(static_cast<float4>(x[i] + r[i]) * scale);
    }
}
