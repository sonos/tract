...
kernel void kernel_mul_mm_nn(
    ...
    device const float * x = (device const float *)(src0
        + args.nb10*(r0*BLOCK_SIZE_M + thread_row) + offset0) + offset1 * 16 * args.ne0;
    ...
    for (int loop_k = 0; loop_k < args.ne00; loop_k += BLOCK_SIZE_K) {
        ...
        for (short i = 0; i < 16; i++) {
            *(sa + SG_MAT_SIZE * ((tiitg/THREAD_PER_ROW/8) \
            +                     (tiitg%THREAD_PER_ROW)*16 + (i/8)*8) \
            +                     (tiitg/THREAD_PER_ROW)%8  + (i&7)*8) = (T) *(x + i * args.ne0);
        }

        *(threadgroup float2x4 *)(sb + 32*8*(tiitg%THREAD_PER_COL) + 8*(tiitg/THREAD_PER_COL)) = *((device float2x4 *) y);

        il = (il + 2 < nl) ? il + 2 : il % 2;
        x += BLOCK_SIZE_K * args.ne0; // Non-adapted old code: (il < 2) ? x + (2 + nl - 1)/nl : x;
        y += BLOCK_SIZE_K;
...
typedef decltype(kernel_mul_mm_nn<half, half4x4, simdgroup_half8x8, float4x4, 1, dequantize_f32>) mat_mm_nn_t;
typedef decltype(kernel_mul_mm_nt<half, half4x4, simdgroup_half8x8, float4x4, 1, dequantize_f32>) mat_mm_nt_t;

template [[host_name("kernel_mul_mm_nn_f32_f32")]]     kernel mat_mm_nn_t kernel_mul_mm_nn<half,   half4x4,   simdgroup_half8x8,   float4x4,      1,     dequantize_f32>;
template [[host_name("kernel_mul_mm_nt_f32_f32")]]     kernel mat_mm_nt_t kernel_mul_mm_nt<half,   half4x4,   simdgroup_half8x8,   float4x4,      1,     dequantize_f32>;