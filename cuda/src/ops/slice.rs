...
mod tests {
    ...
    fn run_test(shape: &[usize], slice: Slice) -> TractResult<()> {
        CUDA_STREAM.with(|stream| {
            ...
            let a_cuda = a.clone().into_device()?.into_tensor().into_tvalue();
            ...
            cpu_output[0].close_enough(
                &cuda_output[0].to_device_tensor()?.to_host()?.into_tensor(),
