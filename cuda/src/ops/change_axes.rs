...
impl Debug for CudaAxisOp {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self.0 {
            AxisOp::Add(a) => write!(f, "CudaAdd({a})"),
            AxisOp::Rm(a) => write!(f, "CudaRm({a})"),
            AxisOp::Move(from, to) => {
                write!(f, "CudaMove({from}, {to})")
            }
...
impl EvalOp for CudaAxisOp {
    ...
    fn eval_with_session(
        ...
        crate::with_cuda_stream(|stream| Memcpy.dispatch_eval(stream, input, 0, &output))?;
        Ok(tvec!(output.into_tensor().into_tvalue()))
    }
}

impl TypedOp for CudaAxisOp {
    as_op!();
    ...
    fn concretize_dims(
        ...
        let op = if let CudaAxisOp(AxisOp::Reshape(axis, from, to)) = self {
            CudaAxisOp(AxisOp::Reshape(
                *axis,
...
pub mod tests {
    ...
    fn test_change_axes() -> TractResult<()> {
        ...
        let output = cuda_model.clone().into_runnable()?.run(tvec![input.clone().into()])?;
