...
fn detect_axis(fact: &TypedFact) -> TractResult<Option<usize>> {
    ...
    match symbolic.len() {
        ...
        _ => bail!(
            "cuda-batched: model has {} symbolic axes in fact {:?}; need exactly one batch axis",
            symbolic.len(),
            fact
        ),
    }
...
impl BatchingRunnable {
    fn wrap(inner: Box<dyn Runnable>) -> TractResult<Box<dyn Runnable>> {
        ...
        let linger_us: u64 =
            std::env::var("TRACT_BATCH_LINGER_US").ok().and_then(|s| s.parse().ok()).unwrap_or(0);
        let max_batch: usize =
            std::env::var("TRACT_BATCH_MAX").ok().and_then(|s| s.parse().ok()).unwrap_or(256);
...
impl Runnable for BatchingRunnable {
    fn spawn(&self) -> TractResult<Box<dyn State>> {
        let tx =
            self.shared.tx.lock().map_err(|_| format_err!("cuda-batched sender poisoned"))?.clone();
...
impl State for BatchingState {
    fn run(&mut self, inputs: TVec<TValue>) -> TractResult<TVec<TValue>> {
        ...
        let (rtx, rrx) = channel();
        self.tx
            .send(Job { inputs, reply: rtx })
            .map_err(|_| format_err!("cuda-batched worker is gone"))?;
        let out =
            rrx.recv().map_err(|_| format_err!("cuda-batched worker dropped the reply"))??;
...
fn worker(
    ...
    loop {
        ...
        match run_batch(&*inner, &jobs, &batch_in, &batch_out) {
            Ok(per_job) => {
                for (job, out) in jobs.into_iter().zip(per_job) {
...
fn run_batch(
    ...
    for (i, axis) in batch_in.iter().enumerate() {
        match axis {
            Some(ax) => {
        ...
        match batch_out.get(o).copied().flatten() {
            Some(ax) => {
                ...
                for (ji, &r) in rows.iter().enumerate() {
                    ...
                    off == t.shape()[ax],
                    "cuda-batched: output rows {} != batch {}",
                    off,
                    t.shape()[ax]
                );
            }
