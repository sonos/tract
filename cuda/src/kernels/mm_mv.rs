...
impl Matmul {
    ...
    pub fn dispatch_eval(
        ...
        if (params.k % 2 == 0) && (params.a_strides[1] % 2 == 0) && params.m == 1 {
            ...
            let cfg = LaunchConfig { 
                ...
                shared_mem_bytes: dbg!((WARP_SIZE * size_of::<f32>()) as u32) };
            unsafe { launch_args.launch(cfg) };
        } else {
            bail!("No kernel found for A: {:?} and B: {:?}", a, b);
        }
    ...
    pub fn eval(
        ...
        let output = unsafe { DeviceTensor::uninitialized_dt(F32, &Self::output_shape(lhs.shape(), rhs.shape()))? };
...
mod tests {
    ...
    fn test_mat_vec() -> TractResult<()> {
        // f32_f32
        run_mmm_test_case((1, 1, 2, 4), F32, F32)?;
        run_mmm_test_case((1, 1, 60, 2), F32, F32)?;
        run_mmm_test_case((2, 1, 128, 7), F32, F32)?;
//
        //// f16_f16
        //run_mmm_test_case::<GgmlGemm>((1, 1, 2, 1), F16, F16)?;
        //run_mmm_test_case::<GgmlGemm>((1, 4, 61, 2), F16, F16)?;
        //run_mmm_test_case::<GgmlGemm>((2, 16, 128, 9), F16, F16)?;
        Ok(())
    }
}