...
use cudarc::driver::{
    CudaFunction, CudaView, CudaViewMut, DeviceRepr, LaunchConfig, PushKernelArg,
};
...
fn dispatch_wgmma_gemm(
    ...
    ensure!(props.major == 9, "WGMMA GEMM is Hopper SM90 only (not SM120 / RTX 5090)");
    ...
    pub fn dispatch_eval(
        ...
        if get_quant_fact(weights, &Q4_0).is_some() {
            ...
            if ctxt.properties().major == 9 && params.k >= 16 {
                dispatch_wgmma_gemm(stream, weights, activs, out, params)?;
            } else {
