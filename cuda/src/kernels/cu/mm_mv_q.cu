...
#if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800)
    // Issue Y0 then Y1 as two cp.async groups. wait_group<1> drains only the
    // oldest (Y0) so Y1 stays in flight during vec_dot(Y0). wait_group<0>
    // is wait-all; using it here would serialize the pipeline.
    {
