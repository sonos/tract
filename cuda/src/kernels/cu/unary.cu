...
static __device__ __forceinline__ __half op_round(__half x) { return (x < (__half)0.0f) ? hceil(x - (__half)0.5f) : hfloor(x + (__half)0.5f); }
...
static __device__ __forceinline__ __half op_sinh(__half x) { return (hexp(x) - hexp(-x)) / (__half)2.0f; }
...
static __device__ __forceinline__ __half op_cosh(__half x) { return (hexp(x) + hexp(-x)) / (__half)2.0f; }
