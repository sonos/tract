...
// 128-byte tensor map (CUtensorMap). Lives here, not in common.cuh: TMA PTX
// must not be compiled into every NVRTC library. FlashAttn is first in the
// eager compile list, so a ptxas reject would fail context init and take
// every CUDA test with it.
