...
template <int STRIDE>
static __device__ __forceinline__ uint32_t swizzle(uint32_t index) {
    // no need swizzling
...
static __device__ inline void cp_async_cg_16B_pred(uint32_t dst,
                                                   const void *src, bool pred) {
    ...
    if (pred) {
        ...
        int z = 0;
        asm volatile("st.shared.v4.b32 [%0], {%1,%1,%1,%1};" ::"r"(dst),
                     "r"(z));
    }
#endif
}
...
global_to_shared_swizzle(uint32_t dst, const half *__restrict__ src,
...
template <int HEIGHT, int WIDTH, int TB_SIZE>
static __device__ __forceinline__ void
global_to_shared_swizzle_pred(uint32_t dst, const half *__restrict__ src,
...
// Tensor Core helpers
// ============================================================================
static __device__ __forceinline__ void ldmatrix_x4(uint32_t regs[4],
                                                   uint32_t addr) {
    asm volatile(
...
static __device__ __forceinline__ void ldmatrix_x4_trans(uint32_t regs[4],
                                                         uint32_t addr) {
    asm volatile(
...
static __device__ __forceinline__ void mma_m16n8k16(uint32_t A[4],
                                                    uint32_t B[2], float D[4]) {
...
static __device__ __forceinline__ void
st_global_half2_pred(half2 *addr, half2 val, bool pred) {
...
template <int BLOCK_Q, int BLOCK_KV, int DIM, int NUM_WARPS, bool is_causal,
          bool use_mask, bool full_q_tile, bool kv_tail>
static __device__ __forceinline__ void
kv_iter_body(const int kv_tile_base, const int len_q, const int len_kv,
             const int past, const int q_block_base, const int warp_id,
             const int lane_id, float scale, const half *__restrict__ MaskBase,
    ...
    _Pragma("unroll") for (int qi = 0; qi < WARP_Q / MMA_M; ++qi) {
        ...
        this_rowmax0 =
            fmaxf(this_rowmax0, __shfl_xor_sync(0xFFFFFFFF, this_rowmax0, 1));
        this_rowmax0 =
            fmaxf(this_rowmax0, __shfl_xor_sync(0xFFFFFFFF, this_rowmax0, 2));
        this_rowmax1 =
            fmaxf(this_rowmax1, __shfl_xor_sync(0xFFFFFFFF, this_rowmax1, 1));
        this_rowmax1 =
            fmaxf(this_rowmax1, __shfl_xor_sync(0xFFFFFFFF, this_rowmax1, 2));
...
// ============================================================================
template <int BLOCK_Q, int BLOCK_KV, int DIM, int NUM_WARPS, bool is_causal,
          bool use_mask, bool full_q_tile, bool kv_rem>
static __device__ void
attention_v5_kernel(const half *__restrict__ Q, // [bs, len_q, DIM]
    ...
    uint32_t Q_smem_thread, K_smem_thread, V_smem_thread;
    {
        ...
        Q_smem_thread = swizzle<DIM * sizeof(half)>(
            Q_smem + (row_off * DIM + col_off) * sizeof(half));
    }
    {
        const int row_off = lane_id % 8;
        const int col_off = (lane_id / 8) * 8;
        K_smem_thread = swizzle<DIM * sizeof(half)>(
            K_smem + (row_off * DIM + col_off) * sizeof(half));
    }
    {
        const int row_off = lane_id % 16;
        const int col_off = (lane_id / 16) * 8;
        V_smem_thread = swizzle<DIM * sizeof(half)>(
            V_smem + (row_off * DIM + col_off) * sizeof(half));
    }
    ...
    if constexpr (full_q_tile) {
        ...
        global_to_shared_swizzle_pred<BLOCK_Q, DIM, TB_SIZE>(Q_smem, Qptr, DIM,
                                                             tid, q_valid);
    }
    ...
    if (kv_full_iters > 0) {
        global_to_shared_swizzle<BLOCK_KV, DIM, TB_SIZE>(K_smem + 0, Kcur, DIM,
                                                         tid);
    ...
    for (int kv_id = 0; kv_id < kv_full_iters; ++kv_id) {
        const int kv_tile_base =
            kv_id * BLOCK_KV; // columns start for this tile

        // Prefetch V (unguarded)
        __syncthreads();
        global_to_shared_swizzle<BLOCK_KV, DIM, TB_SIZE>(V_smem, Vcur, DIM,
                                                         tid);
...

        {
            ...
            if (kv_id + 1 < kv_full_iters) {
                const uint32_t Kdst =
                    K_smem +
                    ((kv_id + 1) % 2) * (BLOCK_KV * DIM * sizeof(half));
                global_to_shared_swizzle<BLOCK_KV, DIM, TB_SIZE>(Kdst, Kcur,
                                                                 DIM, tid);
                Kcur += (size_t)BLOCK_KV * DIM;
                asm volatile("cp.async.commit_group;");
            }

            kv_iter_body<BLOCK_Q, BLOCK_KV, DIM, NUM_WARPS, is_causal, use_mask,
                         full_q_tile, false>(kv_tile_base, len_q, len_kv, past,
                                             q_block_base, warp_id, lane_id,
                                             scale, MaskBase, S_rmem, P_rmem,
                                             O_rmem, rowmax, rowsumexp);
        }
    ...
    if constexpr (kv_rem) {
        ...
        const uint32_t Kdst =
            K_smem + (kv_full_iters % 2) * (BLOCK_KV * DIM * sizeof(half));
        global_to_shared_swizzle_pred<BLOCK_KV, DIM, TB_SIZE>(
            Kdst, Kcur, DIM, tid, len_kv % BLOCK_KV);
        Kcur += (size_t)BLOCK_KV * DIM;
        asm volatile("cp.async.commit_group;");
        ...
        global_to_shared_swizzle_pred<BLOCK_KV, DIM, TB_SIZE>(
            V_smem, Vcur, DIM, tid, len_kv % BLOCK_KV);
        Vcur += (size_t)BLOCK_KV * DIM;
        asm volatile("cp.async.commit_group;");
...

        {
            ...
            kv_iter_body<BLOCK_Q, BLOCK_KV, DIM, NUM_WARPS, is_causal, use_mask,
                         full_q_tile, true>(kv_tile_base, len_q, len_kv, past,
                                            q_block_base, warp_id, lane_id,
                                            scale, MaskBase, S_rmem, P_rmem,
                                            O_rmem, rowmax, rowsumexp);
        }
...
#define INSTANTIATE_FLASH_ATTN_FOR_MASK_STRATEGY(BLOCK_Q, BLOCK_KV, D,         \
                                                 is_causal, use_mask)          \
    extern "C" {                                                               \
    __launch_bounds__(4 * WARP_SIZE) __global__ void attention_v5_full_        \
        ##BLOCK_Q##_##BLOCK_KV##_##D##_##is_causal##_                          \
        ##use_mask(const half *__restrict__ Q, const half *__restrict__ K,     \
...
#define INSTANTIATE_FLASH_ATTN_FOR_D(block_q, block_kv, D)                     \
    INSTANTIATE_FLASH_ATTN_FOR_MASK_STRATEGY(block_q, block_kv, D, false,      \
                                             false)                            \
    INSTANTIATE_FLASH_ATTN_FOR_MASK_STRATEGY(block_q, block_kv, D, false,      \
                                             true)                             \
