...
{% else %}
  {% set T = "__half" %}
  {% set load = "__half2float(" %}
  {% set store = "__float2half(" %}
{% endif %}

{% for georank in range(1, 5) %}

extern "C" __global__ void conv{{georank}}d_{{type}}_generic(
    const {{T}} *input,
    int32_t in_n, int32_t in_c,
    {% for i in range(1, georank + 1) %} int32_t in_{{i}}, {% endfor %}
    int32_t in_n_stride, int32_t in_c_stride,
    {% for i in range(1, georank + 1) %} int32_t in_{{i}}_stride, {% endfor %}

    const {{T}} *kernel,
    int32_t groups, int32_t co_per_group, int32_t ci_per_group,
    {% for i in range(1, georank + 1) %} int32_t ker_{{i}}, {% endfor %}
    int32_t ker_g_stride, int32_t ker_o_stride, int32_t ker_i_stride,
    {% for i in range(1, georank + 1) %} int32_t ker_{{i}}_stride, {% endfor %}

    const {{T}} *bias,
    int32_t bias_stride,

    {% for i in range(1, georank + 1) %} int32_t pad_{{i}}, {% endfor %}
    {% for i in range(1, georank + 1) %} int32_t stride_{{i}}, {% endfor %}
    {% for i in range(1, georank + 1) %} int32_t dil_{{i}}, {% endfor %}

    {{T}} *output,
    int32_t out_n, int32_t out_c,
    {% for i in range(1, georank + 1) %} int32_t out_{{i}}, {% endfor %}
    int32_t out_n_stride, int32_t out_c_stride
    {% for i in range(1, georank + 1) %}, int32_t out_{{i}}_stride {% endfor %}
) {
  ...
  {% for i in range(1, georank) | reverse %}
  ...
  {% endfor %}
        ...
        {% for i in range(1, georank + 1) %} + x_{{i}} * in_{{i}}_stride {%endfor%}){% if type == "f16" %}){% endif %};
        float k = {{load}}*(pfk + ci * ker_i_stride +
        {% for i in range(1, georank + 1) %} + k_{{i}} * ker_{{i}}_stride {%endfor%}){% if type == "f16" %}){% endif %};
        sum += i*k;
    {% for i in range(1, georank + 1) %} } {%endfor%} // nested georank loops
  } // ci loop

  size_t poffset = n * out_n_stride + co * out_c_stride
      {% for i in range(1, georank + 1) %} + ox_{{i}} * out_{{i}}_stride {%endfor%};
  {% if type == "f16" %}
