...
/// Multi-node ops (PrefixMatMul, Sdpa, Conv, Const) are handled separately.
fn try_make_cuda_op(
    source: &TypedModel,
    node: &TypedNode,
) -> TractResult<Option<Box<dyn TypedOp>>> {
    let input_facts = source.node_input_facts(node.id)?;
    let input_dts: Vec<_> = input_facts.iter().map(|f| f.datum_type).collect();
    ...
    if let Some(op) = node.op_as::<ElementWiseOp>() {
        ...
        if crate::kernels::element_wise::is_supported(&*op.0, input_dts[0]) {
            return Ok(Some(Box::new(cuda_element_wise_op(op.0.clone()))));
        }
        return Ok(None);
    }
    ...
    if let Some(op) = node.op_as::<TypedBinOp>() {
        if crate::kernels::binary::is_supported(&*op.0, input_dts[0]) {
            return Ok(Some(Box::new(cuda_bin_op(op.0.clone()))));
        }
        return Ok(None);
    }
    ...
    if let Some(op) = node.op_as::<Cast>() {
        return Ok(cuda_cast_new(op.to).map(|c| Box::new(c) as _));
    }
    ...
    if let Some(op) = node.op_as::<Reduce>() {
        if let Ok(gpu_op) = GpuReduce::from_tract_core(op, "Cuda", cuda_reduce_launch) {
    ...
    if let Some(op) = node.op_as::<Delay>() {
        return Ok(Some(Box::new(tract_gpu::ops::pulse::GpuDelay::new(
            op,
            "Cuda",
            crate::kernels::array::cuda_copy_nd_dispatch,
        ))));
    ...
    if let Some(op) = node.op_as::<PulsePad>() {
        return Ok(Some(Box::new(tract_gpu::ops::pulse::GpuPulsePad::new(
            op,
            "Cuda",
            crate::kernels::array::cuda_copy_nd_dispatch,
        )?)));
