//! End-to-end CUDA arena benchmark with a configurable active-expert path.
//!
//! Compare this benchmark between a base checkout and the arena-enabled
//! checkout; CUDA enables the arena whenever sizing hints are supplied.
...
fn main() -> TractResult<()> {
    ...
    let runtime = tract_core::runtime::runtime_for_name("cuda")?
        .context("CUDA runtime was not registered")?;
    let options =
        RunOptions { memory_sizing_hints: Some(SymbolValues::default()), ..Default::default() };
    let runnable = runtime.prepare_with_options(build_model(layers, experts, m, k)?, &options)?;
    ...
    println!(
        "layers={layers} active_experts={experts} shape=1x{m}x{k} samples={samples} median_ms={:.3} mean_ms={:.3}",
