...
pub fn register_all_ops(reg: &mut OnnxOpRegister) {
    ...
    reg.insert("Elu", elu);
    reg.insert("GlobalAveragePool", |_, _| Ok(Box::new(tractops::nn::GlobalAvgPool::default())));
    reg.insert("GlobalLpPool", global_lp_pool);
    reg.insert("GlobalMaxPool", |_, _| Ok(Box::new(tractops::nn::GlobalMaxPool::default())));
    ...
    reg.insert("PRelu", |_, _| Ok(Box::new(Prelu::default())));
    ...
    reg.insert("Relu", |_, _| Ok(Box::new(tractops::nn::Relu::default())));
    reg.insert("ScaledTanh", scaled_tanh);
    reg.insert("Shrink", shrink);
    reg.insert("ThresholdedRelu", thresholded_relu);
    reg.insert("Selu", selu);
    reg.insert("Sigmoid", |_, _| Ok(Box::new(tractops::nn::Sigmoid::default())));
    reg.insert("Softmax", layer_soft_max);
    reg.insert("Softplus", |_, _| Ok(Box::new(tractops::nn::Softplus::default())));
    reg.insert("Softsign", |_, _| Ok(Box::new(tractops::nn::Softsign::default())));
}
...
pub fn arg_max_min(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn batch_normalization(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn conv(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn average_pool(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn elu(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let alpha = node.get_attr_opt("alpha")?.unwrap_or(1.);
    Ok(Box::new(tractops::nn::Elu::new(alpha)))
}

pub fn global_lp_pool(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let p: usize = node.get_attr_opt("p")?.unwrap_or(2);
    Ok(Box::new(tractops::nn::GlobalLpPool::new(p)))
}

pub fn hard_sigmoid(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn layer_hard_max(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let axis = node.get_attr_opt("axis")?.unwrap_or(1);
    Ok(Box::new(tractops::nn::LayerHardmax::new(axis)))
}

pub fn layer_log_soft_max(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let axis = node.get_attr_opt("axis")?.unwrap_or(1);
    Ok(Box::new(tractops::nn::LayerLogSoftmax::new(axis)))
}

pub fn layer_soft_max(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let axis = node.get_attr_opt("axis")?.unwrap_or(1);
    Ok(Box::new(tractops::nn::LayerSoftmax::new(axis)))
}

pub fn leaky_relu(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let alpha = node.get_attr_opt("alpha")?.unwrap_or(0.01);
    Ok(Box::new(tractops::nn::LeakyRelu::new(alpha)))
}

pub fn lrn(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn max_pool(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let kernel_shape = node.get_attr_tvec("kernel_shape")?;
    let pad = pad(node)?;
    let strides = strides(node)?;
...
pub fn parametric_softplus(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn scaled_tanh(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let alpha = node.get_attr("alpha")?;
    let beta = node.get_attr("beta")?;
...
pub fn shrink(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn selu(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
...
pub fn thresholded_relu(_ctx: &ParsingContext, node: &NodeProto) -> TractResult<Box<InferenceOp>> {
    let alpha = node.get_attr_opt("alpha")?.unwrap_or(1.);
