...
impl BatchNorm {
    ...
    where
        ...
        f32: AsPrimitive<T>,
    {
        let scale = scale.try_as_dense()?.to_array_view::<T>()?.into_shape_with_order((c_dim,))?;
        let beta = beta.try_as_dense()?.to_array_view::<T>()?.into_shape_with_order((c_dim,))?;
        let mean = mean.try_as_dense()?.to_array_view::<T>()?.into_shape_with_order((c_dim,))?;
        let var = var.try_as_dense()?.to_array_view::<T>()?.into_shape_with_order((c_dim,))?;
