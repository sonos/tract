...
impl BatchNorm {
    ...
    where
        ...
        f32: AsPrimitive<T>,
    {
        let scale = scale.to_dense_array_view::<T>()?.into_shape_with_order((c_dim,))?;
        let beta = beta.to_dense_array_view::<T>()?.into_shape_with_order((c_dim,))?;
        let mean = mean.to_dense_array_view::<T>()?.into_shape_with_order((c_dim,))?;
        let var = var.to_dense_array_view::<T>()?.into_shape_with_order((c_dim,))?;
