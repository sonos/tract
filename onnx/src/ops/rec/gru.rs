    ...
    fn wire_body(&self, prefix: &str, body: &mut TypedModel) -> TractResult<()> {
        ...
        let rt_Ht_1_RhT_Rbh = if self.linear_before_reset {
            ...
            let Ht_1_RhT_Rbh = if let Some(b) = b {
                ...
                wbin!(Ht_1_RhT_Rbh = math::Add, Ht_1_RhT, Rbh);
                Ht_1_RhT_Rbh
            } else {
                Ht_1_RhT
            };
            wbin!(rt_Ht_1_RhT_Rbh = math::Mul, rt, Ht_1_RhT_Rbh);
            rt_Ht_1_RhT_Rbh
        } else {
            // (rt (.) Ht-1)*(Rh^T) + Rbh
            wbin!(rt_Ht_1 = math::Mul, rt, Ht_1);
            wire!(rt_Ht_1_RhT = matmul_t, rt_Ht_1, Rh);
            if let Some(b) = b {
                wire!(Rbh = array::Slice::new(1, 5.to_dim() * &h_size, 6.to_dim() * &h_size), b);
                wbin!(rt_Ht_1_RhT_Rbh = math::Add, rt_Ht_1_RhT, Rbh);
                rt_Ht_1_RhT_Rbh
            } else {
                rt_Ht_1_RhT
            }
        };
        wbin!(ht0 = math::Add, Xt_WhT, rt_Ht_1_RhT_Rbh);
        let mut ht0 = ht0;
        if let Some(b) = b {
            wire!(Wbh = array::Slice::new(1, 2.to_dim() * &h_size, 3.to_dim() * &h_size), b);
            wbin!(ht0_biased = math::Add, ht0, Wbh);
            ht0 = ht0_biased
        }
        ...
        wbin!(one_sub_zt = math::Sub, one, zt);
        wbin!(one_sub_zt_ht = math::Mul, one_sub_zt, ht);
        wbin!(zt_Ht_1 = math::Mul, zt, Ht_1);
        wbin!(Ht = math::Add, one_sub_zt_ht, zt_Ht_1);
