...
pub fn create_sdpa_mask_graph(model: &TypedModel, node: &TypedNode, node_name: &str, op: &Sdpa, dt: DatumType, mode: SdpaMaskMode) -> TractResult<Option<TypedModelPatch>>{
    ...
    let s_range = patch.wire_node("mask_s_range", Range::new(s.clone()), &[p_outlet, s_plus_p_outlet, range_increment])?;
    let s_plus_p_range = patch.wire_node("mask_s+p_range", Range::new(s_plus_p.clone()), &[zero, s_plus_p_outlet, range_increment])?;
    let s_range_add_axis = patch.wire_node("mask_s_range.add_axis", AxisOp::Add(1), &s_range)?[0];
    let s_plus_p_range_add_axis = patch.wire_node("mask_s_plus_p_range.add_axis", AxisOp::Add(0), &s_plus_p_range)?[0];

    let greater = patch.wire_node("mask.greater", Comp::GT, &[s_plus_p_range_add_axis, s_range_add_axis])?[0];
    let cast_greater = patch.wire_node("mask.greater.cast", Cast::new(dt), &[greater])?[0];

    let multiplier = match mode {
        SdpaMaskMode::Causal => {
            patch.add_const("P", dt.min_value())?
        },
        SdpaMaskMode::Neutral => {
            patch.add_const("P", Tensor::zero_scalar_dt(dt)?)?
        }
    };
    ...
    let mask = patch.wire_node("mask", TypedBinOp(Box::new(tract_core::ops::math::Mul), None), &[cast_greater, reshaped_mult])?[0];

    let mut inputs = node.inputs.iter().map(|inp| patch.tap_model(model, *inp)).collect::<TractResult<Vec<_>>>()?;
    inputs.push(mask);
