...
pub fn plug(ops: &mut Ops) {
    if is_cortex_a53().unwrap_or(false) {
        ...
        ops.mmv_f32 = Box::new(|m, k| {
            Box::new(MatMatMulImpl::<arm64simd::MatMatMulF32x64x1A53, f32>::new(m, k, 1))
        });
        ops.mmm_f32 = Box::new(|m, k, n| {
            if m >= 128 || m.div_ceil(12) * 12 <= m.div_ceil(8) * 8 {
                Box::new(MatMatMulImpl::<arm64simd::MatMatMulF32x12x8A53, f32>::new(m, k, n))
            } else {
                Box::new(MatMatMulImpl::<arm64simd::MatMatMulF32x8x8A53, f32>::new(m, k, n))
            }
        })
    } else {
        log::info!("arm64simd activated for smmm (generic)");
        ops.mmv_f32 = Box::new(|m, k| {
            Box::new(MatMatMulImpl::<arm64simd::MatMatMulF32x64x1, f32>::new(m, k, 1))
        });
        ops.mmm_f32 = Box::new(|m, k, n| {
            if m >= 128 || m.div_ceil(12) * 12 <= m.div_ceil(8) * 8 {
                Box::new(MatMatMulImpl::<arm64simd::MatMatMulF32x12x8, f32>::new(m, k, n))
            } else {
                Box::new(MatMatMulImpl::<arm64simd::MatMatMulF32x8x8, f32>::new(m, k, n))
            }
        })
    }
    ops.qmmm_i8_i8 = Box::new(|m, k, n| {
        Box::new(MatMatMulImpl::<arm64simd::MatMatMulI8x8x8, i32>::new(m, k, n))
    });
    ops.qmmv_i8_i8 =
        Box::new(|m, k| Box::new(MatMatMulImpl::<arm64simd::MatMatMulI8x64x1, i32>::new(m, k, 1)));
    ops.qmmm_i8_i32 = Box::new(|m, k, n| {
        Box::new(MatMatMulImpl::<arm64simd::MatMatMulI8xI32x8x8, i32>::new(m, k, n))
    });
    ops.qmmv_i8_i32 = Box::new(|m, k| {
        Box::new(MatMatMulImpl::<arm64simd::MatMatMulI8xI32x64x1, i32>::new(m, k, 1))
    });
    ops.sigmoid_f32 =
