...
pub fn pool_for(target: crate::platform::Target) -> Vec<Box<dyn MatMatMul>> {
    let mut pool: Vec<Box<dyn MatMatMul>> = all()
        ...
        .map(|r| (r.make)())
        // A kernel this build compiled has to be runnable here to be in a pool; one it did
        // not compile cannot answer that, so it stays as metadata.
        .filter(|kernel| !kernel.built() || kernel.is_supported_here())
        .collect();
...
pub fn pool() -> Vec<Box<dyn MatMatMul>> {
    let mut pool: Vec<Box<dyn MatMatMul>> =
        all().map(|r| (r.make)()).filter(|k| k.is_supported_here()).collect();
    pool.sort_by(|a, b| a.name().cmp(b.name()));
    pool
}
