    ...
    TI: LADatum + Bounded + PartialEq,
{
    ...
    let pattern = tensor1(&(0..).take(len).collect_vec()).cast_to::<TI>().unwrap().into_owned();
    let result = vec![TC::max_value(); len];
    ...
    let non_linear = tvec![
        unsafe { FusedKerSpec::LoadTileColMajor(pattern.as_ptr_unchecked()) },
        FusedKerSpec::Store(OutputStoreKer {
            ptr: result.as_ptr() as _,
            row_byte_stride,
            col_byte_stride,
    ...
    let err = ker.kernel(&non_linear);
    assert_eq!(err, 0);
    let mut expected = pattern.cast_to::<TC>().unwrap().into_owned();
    if row_major {
        expected =
            expected.into_shape(&[ker.nr(), ker.mr()]).unwrap().permute_axes(&[1, 0]).unwrap();
    }
