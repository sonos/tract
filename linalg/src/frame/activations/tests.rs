...
pub fn max_const<T: LADatum>(c: T) -> Program<T> {
    Program { ops: vec![MaxConst(0), Done], csts: vec![c] }
}
        ...
        proptest::proptest! {
                ...
                $($param in proptest::prelude::any::<$ti>()),*)
            {
                ...
                if $cond {
                    ...
                    <$ker>::run(&prog.ops, &prog.csts, input.as_slice_mut::<$ti>().unwrap());
...
macro_rules! act_tests {
    ($cond:expr, $ker:ty, $ti:ty) => {
        ...
        fn max_const_0() {
            use crate::frame::activations::ActivationKer;
            if $cond {
                let mut input = tract_data::prelude::Tensor::zero_aligned::<$ti>(&[<$ker>::nr()], <$ker>::alignment_bytes()).unwrap();
                input.fill_t::<$ti>(0.0).unwrap();
                let expected:Vec<$ti> = input.as_slice::<$ti>().unwrap().iter().cloned().map(|x| x.max(0f32)).collect();
                let prog = crate::frame::activations::tests::max_const(0f32);
                <$ker>::run(&prog.ops, &prog.csts, input.as_slice_mut::<$ti>().unwrap());
        ...
        fn max_const_big_alpha() {
            ...
            if $cond {
                let mut input = tract_data::prelude::Tensor::zero_aligned::<$ti>(&[<$ker>::nr()], <$ker>::alignment_bytes()).unwrap();
                input.fill_t::<$ti>(0.0).unwrap();
                let expected:Vec<$ti> = input.as_slice::<$ti>().unwrap().iter().cloned().map(|x| x.max(7.567773e37)).collect();
                let prog = crate::frame::activations::tests::max_const(7.567773e37);
                <$ker>::run(&prog.ops, &prog.csts, input.as_slice_mut::<$ti>().unwrap());
