      ...
      f32: From<F>
{
    fn tract(&self) -> TractResult<TypedModel> {
        ...
        let cos = model.add_source("cos", TypedFact::shape_and_dt_of(&self.cos.clone().into_tensor()))?;
        let sin = model.add_source("sin", TypedFact::shape_and_dt_of(&self.sin.clone().into_tensor()))?;
      ...
      f32: From<F>
{
    fn run_with_approx(
        ...
        let mut output = runtime.prepare(model)?.run(tvec![self.input.clone().into_tvalue(), self.cos.clone().into_tvalue(), self.sin.clone().into_tvalue()])?;
