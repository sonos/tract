...
pub fn register_all_ops(reg: &mut TfOpRegister) {
    ...
    reg.insert("Reshape", |_, _| Ok(Box::new(::tract_hir::array::Reshape::new())));
    reg.insert("Shape", |_, _| Ok(Box::new(::tract_hir::array::Shape::new(DatumType::I32))));
    reg.insert("Slice", |_, _| Ok(Box::new(slice::Slice)));
    reg.insert("Squeeze", squeeze::squeeze);
    reg.insert("StridedSlice", strided_slice);
    reg.insert("Tile", |_, _| Ok(Box::new(::tract_hir::array::Tile)));
...
pub fn strided_slice(_ctx: &ParsingContext, pb: &NodeDef) -> TractResult<Box<dyn InferenceOp>> {
    use tract_hir::array::StridedSlice;
