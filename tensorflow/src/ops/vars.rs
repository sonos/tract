...
impl OpState for VariableV2State {
    fn eval(&mut self, _session: &mut SessionState, op: &Op, _inputs: TVec<SharedTensor>) -> TractResult<TVec<SharedTensor>> {
        ...
        let locked = self
            .0
            .lock()
            .map_err(|_| format!("poisoned lock on variable {}", op.name))?;
...
impl StatefullOp for VariableV2 {
    fn state(&self) -> TractResult<Option<Box<OpState>>> {
        ...
        Ok(Some(Box::new(VariableV2State(Arc::new(Mutex::new(
            tensor,
        ))))))
    }
}
...
impl OpState for AssignState {
    fn eval(
        ...
        let var_state = unsafe {
            (var_state as *const Mutex<Tensor>)
                .as_ref()
                .ok_or("null pointer received from var")?
        };
        println!("assigning {:?}", new);
        let mut lock = var_state
            .lock()
            .map_err(|_| "poisoned lock for assignment")?;
...
mod tests {
    ...
    fn var_assign() {
        ...
        let reset = model
            .add_node("reset".into(), Box::new(Assign::default()))
            .unwrap();
        model
            .add_edge(OutletId::new(var, 0), InletId::new(reset, 0))
            .unwrap();
        model
            .add_edge(OutletId::new(zero, 0), InletId::new(reset, 1))
            .unwrap();
        model
            .add_edge(OutletId::new(var, 1), InletId::new(reset, 2))
            .unwrap();
        let set = model
            .add_node("set".into(), Box::new(Assign::default()))
            .unwrap();
        model
            .add_edge(OutletId::new(var, 0), InletId::new(set, 0))
            .unwrap();
        model
            .add_edge(OutletId::new(one, 0), InletId::new(set, 1))
            .unwrap();
        model
            .add_edge(OutletId::new(var, 1), InletId::new(set, 2))
            .unwrap();
