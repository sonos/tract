...
impl Expansion for FusedBatchNorm {
    ...
    fn wire(
        ...
        {
            let scale = scale.try_as_plain_ram()?.as_slice::<f32>()?;
            let offset = offset.try_as_plain_ram()?.as_slice::<f32>()?;
            let mean = mean.try_as_plain_ram()?.as_slice::<f32>()?;
            let variance = variance.try_as_plain_ram()?.as_slice::<f32>()?;
            let slope: Vec<f32> =
